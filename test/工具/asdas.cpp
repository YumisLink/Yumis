...
int main ()
{
    ...
    // freopen("k","w",stdout);
    // scanf("%d",&n);
    // for (int i = 1; i <= n; i ++){int k; scanf("%d",&k);d_push(k,i);}
    // while(!d_empty()) {printf("%d ",d_front());d_pop();}
    ...
    for (int i = 1; i <= m; i ++)
    {
        ...
        cin(q+1,w+1,r,e);cin(w+1,q+1,r,e);
    }
    if (sun) dij2(st);
    else dij1(st);
    printf("%d %d\n",mlig[ed],dis[ed]);
    ...
    for (int i = 1; i <= m; i ++) ne[i] = 0,to[i] = 0,way[i] = 0,lig[i] = 0,close[i] = 0;
    }
